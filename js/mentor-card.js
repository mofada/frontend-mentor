class MentorCard extends HTMLElement {
	...
	connectedCallback() {
		...
		coverFigure.className =
			"w-full overflow-hidden rounded-lg cursor-pointer border";
		...
		coverImg.alt = title;
		coverImg.className =
			"w-full aspect-video object-cover hover:scale-105 transition-all duration-300";
		...
		// tailwind
		if (isTailwind) {
			let tailwindTag = this.createTag(
				"span",
				"bg-tailwind",
				"Tailwind CSS",
			);
